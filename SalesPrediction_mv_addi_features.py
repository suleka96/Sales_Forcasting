...
def segmentation(data):
    ...
    X = np.array([seq[i: i + config.num_steps_with_lable_features] for i in range(len(seq) -  config.num_steps)])
...
def generate_batches(train_X, train_y, batch_size):
    ...
    for j in batch_indices:
        ...
        assert set(map(len, batch_X)) == {config.num_steps_with_lable_features}
...
def train_test():
    ...
    with config.graph.as_default():
        ...
        inputs = tf.placeholder(tf.float32, [None, config.num_steps_with_lable_features, config.features], name="inputs")
