...
class RNNConfig():
    ...
    test_ratio = 0.2
    fileName = 'store285_1.csv'
    graph = tf.Graph()
    features = 2
    column_min_max = [[0,10000],[1,12]]
    columns = ['Sales', 'Month']
...
def segmentation(data):
    ...
    seq = [price for tup in data[[config.columns[0], config.columns[1]]].values for price in tup]
...
def train_test():
    ...
    with tf.Session(graph=config.graph) as sess:
        ...
        plot(nonescaled_y, pred_vals, "Sales Prediction VS Truth mv4.png")
        write_results(nonescaled_y, pred_vals, "Sales Prediction batch mv4 results.csv")
