...
class RNNConfig():
    ...
    graph = tf.Graph()
    features = 6
    column_min_max = [[0,10000], [1,7], [1,31], [1,12]]
    columns = ['Sales', 'DayOfWeek', 'Day', 'Month', 'SchoolHoliday', 'Promo']
...
def segmentation(data):
    ...
    seq = [price for tup in data[[config.columns[0], config.columns[1], config.columns[2], config.columns[3], config.columns[4], config.columns[5]]].values for price in tup]
