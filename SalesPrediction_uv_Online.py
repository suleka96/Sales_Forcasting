...
def plot(RMSE_mean, prediction_vals, nonescaled_y):
    ...
    plt.plot(days, RMSE_mean, label='RMSE')
    plt.legend(loc='upper left', frameon=False)
    plt.xlabel("day")
    plt.ylabel("RMSE")
    # plt.ylim((min(test_y), max(test_y)))
    plt.grid(ls='--')
    plt.savefig("Sales RMSE mean uv online store 285 .png", format='png', bbox_inches='tight', transparent=False)
    plt.close()
...
def train_test():
    ...
    with tf.Session(graph=graph) as sess:
        ...
        write_results(RMSE_mean,prediction_vals,nonescaled_y,"Sales Online uv results.csv")
