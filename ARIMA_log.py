...
def plot(original_test_list,pred_vals):
    ...
    plt.grid(ls='--')
    plt.savefig("Sales ARIMA log Prediction Vs Truth .png", format='png', bbox_inches='tight', transparent=False)
    plt.close()
...
def preprocess():
    ...
    scaled_data = convert_log(store_data_scale)
...
def ARIMA_model():
    ...
    print("MAPE:", mape)
    rmse_val = RMSPE(test, predictions)
    print("RMSPE:", rmse_val)

    write_results(test, predictions, "ARIMA_log_results.csv")
