...
def pre_process():
    ...
    train_X, train_y = segmentation(log_train_data)

    # -------------- processing test data---------------------------------------

    log_train_data = convert_log(test_data)
    # scaled_test_data = scale(test_data)
    test_X, test_y = segmentation(log_train_data)
...
def write_results(true_vals,pred_vals):
    ...
    with open("RNN_uv_results log.csv", "w") as f:
        writer = csv.writer(f)
...
def train_test():
    ...
    with tf.Session(graph=config.graph) as sess:
        ...
        plot(test_y, pred_vals, "RNN Sales Prediction VS Truth uv log 285.png")
