...
def pre_process():
    ...
    train_size = int(len(store_data) - 61)
...
def train_test():
    ...
    with tf.Session(graph=config.graph) as sess:
        ...
        # pred_vals = convert_from_log(test_pred)
        #
        pred_vals = np.array(test_pred)
        ...
        plot(test_y, pred_vals, "Sales Prediction VS Truth mv log.png")
        write_results(test_y, pred_vals, "Sales Prediction batch mv log results.csv")

        meanSquaredError = mean_squared_error(test_y, pred_vals)
        rootMeanSquaredError = sqrt(meanSquaredError)
        print("RMSE:", rootMeanSquaredError)
        mae = mean_absolute_error(test_y, pred_vals)
        print("MAE:", mae)
        mape = mean_absolute_percentage_error(test_y, pred_vals)
        print("MAPE:", mape)
        rmse_val = RMSPE(test_y, pred_vals)
